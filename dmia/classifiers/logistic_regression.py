...
class LogisticRegression:
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        """
        y_log = X_batch @ self.w
        y_true = np.where(y_batch == 0, -1., 1.)
        loss = np.log(1 + np.exp(y_true * y_log))
        loss = np.sum(loss) / X_batch.shape[0]
        loss += reg * np.sum(self.w[:-1] ** 2)

        dw = X_batch.multiply((y_true / (1 + np.exp(-y_true * y_log)))[:, np.newaxis])
        dw = np.array(np.sum(dw, axis=0)).ravel() / X_batch.shape[0]
        dw += np.concatenate((2 * reg * self.w[:-1], np.zeros(1)))
